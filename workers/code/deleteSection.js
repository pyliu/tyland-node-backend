...
parentPort.on("message", async (data) => {
  ...
  config.isDev && console.log(`收到刪除段小段 ${id} 訊息`, data);
  ...
  try {
    ...
    let foundIdx = -1;
    const found = HXdoc.sections.find((sect, idx) => {
      foundIdx = idx;
      return sect.value === id;
    });
    if (!found) {
      const message =  `⚠ 段小段 ${id} 不存在，無法刪除！`;
      config.isDev && console.log(__basename, message);
      response.statusCode = config.statusCode.FAIL;
      response.message = message;
    } else {
      // remove found index code
      HXdoc.sections.splice(foundIdx, 1);
      // store doc back
      ...
      if (result.acknowledged) {
        ...
        let message =  result.modifiedCount === 0 ? `⚠ 沒有刪除段小段 ${id} 資料!` : `✔ 刪除段小段 ${id} 資料成功。`;
    ...
    console.error(__basename, '❗ 處理刪除段小段資料執行期間錯誤', e);
  } finally {
